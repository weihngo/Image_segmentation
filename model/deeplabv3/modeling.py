...
class deeplabv3plus_xception(nn.Module):
    def __init__(self, in_channels, low_level_channels, num_classes, aspp_dilate=[6, 12, 18]):
        ...
        self.backbone = AlignedXception(BatchNorm=nn.BatchNorm2d, pretrained=False, output_stride=16)
        self.deeplabv3_plus=DeepLabHeadV3Plus(in_channels, low_level_channels, num_classes)


    def forward(self,x):
        x=self.backbone(x)
        x=self.deeplabv3_plus(x)
